...
{
    let actions = [];
    let normalizedPreviousState = JSON.parse(JSON.stringify(previousState));
    let normalizedCurrentState = JSON.parse(JSON.stringify(currentState));
    let difference = diff(normalizedPreviousState, normalizedCurrentState);
...
const getMigration = function(actions, databaseSchema)
{
    ...
    for (let _i in actions)
    {
        ...
        if (databaseSchema) {
            action.tableNameParam = `{
                "tableName": "${action.tableName}",
                "schema": "${databaseSchema}"
            }`;
        } else {
            action.tableNameParam = `"${action.tableName}"`;
        }
        ...
        switch (action.actionType)
        {
            ...
            case 'dropTable':
            {
                let res = `{ fn: "dropTable", params: [${action.tableNameParam}] }`;
                commandsUp.push(res);
...
let resUp = `{ fn: "addColumn", params: [
    ...
    ${propertyToStr(action.options)}
            ...
            case 'removeColumn':
            {
                let res = `{ fn: "removeColumn", params: [${action.tableNameParam}, "${(action.options && action.options.field) ? action.options.field : action.columnName}"] }`;
                commandsUp.push(res);
